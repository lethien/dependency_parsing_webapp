...
def vocab(conll_path):
    ...
    with open(conll_path, encoding="utf8") as conll_file:
...
def parse_sentence_to_conll_entry(sentences, c2i, wordsCount):
    # Character vocabulary
    root = ConllEntry(0, '*root*', '*root*', 'ROOT-POS', 'ROOT-CPOS', '_', -1, 'rroot', '_', '_')
    root.idChars = [1, 2]
    tokens = [root]
    ...
    for sentence in sentences:
        ...
        if not tok or sentence.strip() == '':
            if len(tokens) > 1: yield tokens
            tokens = [root]
        else:
            if sentence[0] == '#' or '-' in tok[0] or '.' in tok[0]:
                tokens.append(sentence.strip())
            else:
                for i in range(len(tok)):
                    entry = ConllEntry(i+1, tok[i], tok[i], None, None, None, None, None, None, None)
